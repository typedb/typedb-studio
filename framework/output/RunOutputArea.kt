...
object RunOutputArea {
    ...
    private fun ToggleButton(state: State) {
        Form.IconButton(
            icon = if (state.isOpen) Icon.Purpose.HIDE else Icon.Purpose.SHOW,
        ...
        Row(modifier.height(PANEL_BAR_HEIGHT), verticalAlignment = Alignment.CenterVertically) {
            ...
            Box(Modifier.weight(1f)) {
                Tabs.Horizontal.Layout(
                    ...
                    closeButtonFn = { IconButtonArg(icon = Icon.Purpose.CLOSE) { mayCloseRunner(it) } },
                    leadingButtonFn = {
                        IconButtonArg(
                            icon = Icon.Purpose.PIN,
