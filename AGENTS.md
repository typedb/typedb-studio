...
- **ngx-resizable drag handles force synchronous layout per mousemove**: `@hhangular/resizable`'s `DragHandleComponent.onMousemove` writes `flexBasis` on both panes and then immediately re-reads `getComputedStyle().width` to detect hitting a limit — write-then-read layout thrash, several full layouts per mousemove (unthrottled, so far more often than once per frame), plus a `MutationObserver` → `percentChange` → change-detection pass. Any heavy DOM inside a resizable pane therefore makes dragging choppy even when that DOM is visually clipped. Fix at the DOM end: don't render what a clipped box won't show (`StaticCodeComponent`'s `preview` input caps history query previews at 40 lines) and add `contain: content` to the clipped block.
