...
object Layout {
    ...
    object Resizable {
        ...
        fun Row(
            ...
            separator: Resizable.Separator? = null,
            vararg items: Resizable.Item
        ) {
            assert(items.size >= 2)
            val pixelDensity = LocalDensity.current.density
            val areaState = remember { Resizable.AreaState(separator?.size) }
        ...
        @Composable
        private fun RowItemResizer(itemState: Resizable.ItemState, separatorWidth: Dp?) {
                ...
                Box(
                    modifier = Modifier.fillMaxHeight()
                        ...
                        .draggable(orientation = Horizontal, state = rememberDraggableState {
                            itemState.tryResizeSelfAndNext(toDP(it, pixelDensity))
                        })
                )
            }
        }
        ...
        @OptIn(ExperimentalComposeUiApi::class)
        @Composable
        private fun ColumnItemResizer(itemState: Resizable.ItemState, separatorHeight: Dp?) {
            if (itemState.freezeSize != null) {
