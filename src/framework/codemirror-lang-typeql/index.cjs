...
// This file was generated by lezer-generator. You probably shouldn't edit it.
const Query = 1, LABEL = 10, COMMA = 11, SUB = 40, OWNS = 54, PLAYS = 59, RELATES = 61, VAR = 68, ClauseMatch = 81, MATCH = 82, Patterns = 83, Statement = 85, StatementAssignment = 86, StatementType = 107, StatementThing = 119, ISA = 123, HAS = 138, LINKS = 140, SEMICOLON = 150, ClauseInsert = 151, INSERT = 152, ClausePut = 153, PUT = 154, ClauseUpdate = 155, UPDATE = 156;

function suggest(type, label, boost = 0) {
...
function isPartOfWord(s) {
    let matches = s.match(/^[A-Za-z0-9_\-\$]+/);
    return matches != null && matches.length > 0;
}
function findStartOfCompletion(context) {
...
function autocompleteTypeQL(context) {
    let tree = language.syntaxTree(context.state);
...
function getSuggestions(context, tree, parseAt) {
...
function climbTillWeRecogniseSomething(context, tree, parseAt, climbedTo, prefix) {
    ...
    switch (climbedTo.type.id) {
        ...
        case ClauseUpdate: { // Ideally not update
            if ([INSERT, UPDATE, PUT].includes((_b = (_a = prefix.at(-1)) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : -1)) {
        ...
        case Patterns: {
            ...
            if (prefix.length == 0 || goodPrefixes.includes((_d = (_c = prefix.at(-1)) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : -1)) {
                let ret = suggestNestedPatterns().concat(suggestVariables(context, tree));
                if (prefix.length > 0 && ((_e = prefix.at(-1)) === null || _e === void 0 ? void 0 : _e.id) == SEMICOLON) {
                    let newPrefix = collectSiblingsOf(climbedTo).concat(prefix);
                    ret = ret.concat((_f = climbTillWeRecogniseSomething(context, tree, parseAt, climbedTo.parent, newPrefix)) !== null && _f !== void 0 ? _f : []);
                }
                return ret;
            }
            else {
        ...
        case Query:
            {
                if (prefix.length == 0 || ((_g = prefix.at(-1)) === null || _g === void 0 ? void 0 : _g.id) == SEMICOLON) {
                    return suggestPipelineStages()
                        .concat(suggestDefinedKeywords());
                }
                else {
                    return null;
                }
...
function collectSiblingsOf(node) {
    let siblings = [];
    let prev = node;
...
function collectPrecedingChildrenOf(context, node) {
...
function suggestLabels(context, tree) {
...
function suggestVariables(context, tree) {
    var options = [];
    tree.iterate({
        enter: (other) => {
            if (other.type.id == VAR) {
...
function suggestThingConstraintKeywords() {
...
function suggestTypeConstraintKeywords() {
...
function suggestDefinedKeywords(context, tree, parseAt, patternsNode, prefix) {
    return ["define", "redefine", "undefine"].map((keyword) => suggest("keyword", keyword, 1));
}
function suggestPipelineStages(context, tree, parseAt, patternsNode, prefix) {
    return ["match", "insert", "delete", "update", "put", "select", "reduce", "sort", "limit", "offset", "end"].map((keyword) => suggest("keyword", keyword, 1));
}
function suggestNestedPatterns(context, tree, parseAt, patternsNode, prefix) {
    return ["not {};", "{} or {};", "try {};"].map((keyword) => suggest("method", keyword, 1));
}
function suggestStatement(context, tree, parseAt, statementNode, prefix) {
...
function suggestStatementRefinedImpl(context, tree, parseAt, refinedStatementNode, prefix) {
    var _a;
    if ((refinedStatementNode === null || refinedStatementNode === void 0 ? void 0 : refinedStatementNode.name) == "StatementAssignment") {
        return null;
    }
    else if (((_a = prefix.at(-1)) === null || _a === void 0 ? void 0 : _a.id) == VAR) {
...
function suggestStatementThing(context, tree, parseAt, refinedStatementNode, prefix) {
    var _a;
    switch ((_a = prefix.at(-1)) === null || _a === void 0 ? void 0 : _a.id) {
        case ISA:
        case HAS: {
...
function suggestStatementType(context, tree, parseAt, refinedStatementNode, prefix) {
    var _a;
    switch ((_a = prefix.at(-1)) === null || _a === void 0 ? void 0 : _a.id) {
        case SUB:
        case OWNS: {
    ...
    parser: parser.configure({
        props: [
            ...
            highlight.styleTags({
                ...
                INTEGERLITERAL: highlight.tags.number,
                DOUBLELITERAL: highlight.tags.number,
                BOOLEANLITERAL: highlight.tags.bool,
                // Types
                LABEL: highlight.tags.typeName,
                BOOLEAN: highlight.tags.typeName,
                INTEGER: highlight.tags.typeName,
                DOUBLE: highlight.tags.typeName,
                DECIMAL: highlight.tags.typeName,
                DATETIMETZ: highlight.tags.typeName,
                DATETIME: highlight.tags.typeName,
                DATE: highlight.tags.typeName,
                DURATION: highlight.tags.typeName,
                STRING: highlight.tags.typeName,
                // Keywords
                ISA: highlight.tags.keyword,
                HAS: highlight.tags.keyword,
                LINKS: highlight.tags.keyword,
                OWNS: highlight.tags.keyword,
                RELATES: highlight.tags.keyword,
                PLAYS: highlight.tags.keyword,
                FUN: highlight.tags.keyword,
                LET: highlight.tags.keyword,
                FIRST: highlight.tags.keyword,
                LAST: highlight.tags.keyword,
                // Value type names?
                // Stages
                DEFINE: highlight.tags.heading1,
                UNDEFINE: highlight.tags.heading1,
                REDEFINE: highlight.tags.heading1,
                MATCH: highlight.tags.heading1,
                INSERT: highlight.tags.heading1,
                DELETE: highlight.tags.heading1,
                UPDATE: highlight.tags.heading1,
                PUT: highlight.tags.heading1,
                END: highlight.tags.heading1,
                SELECT: highlight.tags.heading1,
                REDUCE: highlight.tags.heading1,
                SORT: highlight.tags.heading1,
                OFFSET: highlight.tags.heading1,
                LIMIT: highlight.tags.heading1,
                REQUIRE: highlight.tags.heading1,
                DISTINCT: highlight.tags.heading1,
                GROUPBY: highlight.tags.heading1,
                // SubPattern
...
function typeqlAutocompleteExtension() {
    return autocomplete.autocompletion({ activateOnTypingDelay: 100, override: [autocompleteTypeQL] });
}
...
exports.typeqlAutocompleteExtension = typeqlAutocompleteExtension;
